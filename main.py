...
def format_vcp_message(record: pd.Series, *, rank_no: int) -> str:
    ...
    volume_dry_up_pct = record_float(record, "volume_dry_up_pct")
    quality_line = f"⭐ 품질: {quality_grade}등급 / {vcp_score:.0f}점"
    if volume_dry_up_pct is not None:
        quality_line += f" | 거래량 감소 {volume_dry_up_pct:.1f}%"
